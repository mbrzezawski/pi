...
void bernoulli_gen(int v[], int n, double probability) {
	for(int i = 0; i < n; i++){
		int liczba = rand_from_interval(0, RAND_MAX);
...
void pmf(double v[], int trials) {
	for(int i = 0; i < trials; i++){
		int liczba1 = rand_from_interval(1, 6);
		int liczba2 = rand_from_interval(1, 6);
		int suma = liczba1 + liczba2;
		v[suma] += (double)1/trials;
	}
...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
	for(int i = x_start; i < 13; i++){
		printf("%-2d", i);
		printf(" %c", '|');
		int ilosc = round(v[i] / y_scale);
...
void monty_hall(int n, int *p_switch_wins) {
	int switch_wins = 0;
	for(int i = 0; i < n; i++){
		int door_w = rand_from_interval(1, 3);
		int door_ch = rand_from_interval(1, 3);
		if(door_w != door_ch){
			*p_switch_wins += 1;
		}
	}
